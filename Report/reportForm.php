...
<body>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const minDate = new Date(currentYear, 0, 1); // January is month 0
    const minDateString = minDate.toISOString().split('T')[0];

    // Set the maximum date to today
    const maxDateString = today.toISOString().split('T')[0];

    reportDateInput.min = minDateString;
    reportDateInput.max = maxDateString;
      ...
      form.addEventListener('submit', function(e) {
        ...
        if (selectedDate < minDate || selectedDate > today) {
          ...
          document.querySelector('.error-StartDate').textContent =
            `Date must be between January 1, ${currentYear} and today.`;
        } else {
          document.querySelector('.error-StartDate').textContent = '';
        }
      });
